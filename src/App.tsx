...
export default function App() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-prestige-bg text-prestige-ink">
      <nav className="fixed inset-x-0 top-0 z-50 bg-prestige-bg/95 backdrop-blur border-b border-prestige-divider px-5 md:px-12 py-5 flex items-center justify-between">
        <a href="#hero" className="block">
          <span className="block text-[9px] uppercase tracking-[0.22em] font-bold text-prestige-ink/40">Bienestar corporativo · FUNDAE · Mallorca</span>
          <span className="font-serif text-xl">Juan Luis Rodríguez</span>
        </a>
      ...
      {open && (
        <div className="fixed inset-0 z-40 bg-prestige-bg pt-28 px-6 lg:hidden">
          <div className="flex flex-col items-center gap-8">
            ...
            <a href={wa} target="_blank" rel="noopener" className="btn-primary">Solicitar diagnóstico</a>
          </div>
        </div>
      )}

      <section id="hero" className="min-h-screen pt-28 md:pt-32 border-b border-prestige-divider grid lg:grid-cols-12">
        <div className="lg:col-span-7 section-padding flex flex-col justify-center border-r border-prestige-divider">
          ...
          <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl font-light leading-[1.05] mb-8 max-w-4xl">Reduce bajas, mejora el clima y consigue que tu equipo quiera quedarse.</h1>
          <p className="text-lg md:text-xl text-prestige-ink/70 font-light leading-relaxed mb-8 max-w-2xl">Diseño formaciones y experiencias corporativas prácticas para cuidar la salud física, mental y relacional de tus trabajadores. Sin actividades de relleno. Con una propuesta clara y adaptada a tu empresa.</p>
          <div className="flex flex-col sm:flex-row gap-4 mb-10">
            <a href={wa} target="_blank" rel="noopener" className="btn-primary justify-center"><MessageCircle className="w-4 h-4" /> Solicitar diagnóstico 360º gratuito</a>
            <a href="#servicios" className="btn-outline justify-center">Ver servicios</a>
          </div>
          <div className="grid sm:grid-cols-2 gap-4 max-w-2xl">
            {["Diagnóstico 360º gratuito", "Propuesta a medida en 48h", "Formaciones bonificables", "Presencial en Mallorca y online"].map(item => <div key={item} className="flex items-center gap-3 text-sm text-prestige-ink/65"><CheckCircle2 className="w-4 h-4 text-prestige-blue" />{item}</div>)}
          </div>
        </div>
        <div className="lg:col-span-5 bg-white/40 p-8 md:p-12 lg:p-16 flex flex-col justify-center gap-8">
          <div className="relative aspect-[4/5] bg-prestige-accent rounded-[2rem] overflow-hidden shadow-inner">
            ...
            <div className="absolute bottom-8 left-0 right-0 px-8 text-white"><p className="text-[10px] uppercase tracking-[0.3em] font-bold mb-4">Juanlu Rodríguez · Mallorca</p><p className="font-serif text-2xl italic leading-tight">Salud, cohesión y rendimiento para empresas que quieren cuidar a su gente de verdad.</p></div>
          </div>
          <div className="bg-prestige-ink text-prestige-bg rounded-[1.5rem] p-7 shadow-2xl"><p className="font-serif text-2xl leading-tight mb-4">No pierdes solo horas por bajas. Pierdes energía, compromiso, clima y talento.</p><p className="text-sm text-prestige-bg/65">El bienestar bien diseñado es una herramienta empresarial.</p></div>
        </div>
      </section>
      ...
      <section id="contacto" className="section-padding bg-prestige-accent/10 scroll-mt-24"><div className="container mx-auto grid lg:grid-cols-2 gap-14 items-center"><div><Label>Contacto</Label><Title>Empecemos por detectar dónde está perdiendo energía tu equipo.</Title><p className="text-lg text-prestige-ink/70 mb-8">Escríbeme y te preparo el primer paso: diagnóstico 360º, lectura del contexto y recomendación de acciones.</p><div className="grid sm:grid-cols-2 gap-6"><a href="tel:+34640761701" className="font-serif text-xl"><Phone className="inline w-5 h-5 mr-2 text-prestige-blue" />640 761 701</a><a href="mailto:contacto@juanlurodriguez.com" className="font-serif text-xl"><Mail className="inline w-5 h-5 mr-2 text-prestige-blue" />Email directo</a></div></div><div className="bg-prestige-ink text-prestige-bg p-9 md:p-14 rounded-[2.5rem] shadow-2xl"><p className="font-serif text-3xl md:text-4xl font-light leading-tight mb-8">Envíame un WhatsApp y dime cuántos trabajadores tiene tu empresa.</p><a href={wa} target="_blank" rel="noopener" className="btn-primary w-full justify-center bg-prestige-bg text-prestige-ink hover:bg-white"><MessageCircle className="w-4 h-4" /> Solicitar diagnóstico</a><p className="text-xs text-prestige-bg/45 mt-6 italic">Propuestas sujetas a crédito disponible y requisitos aplicables.</p></div></div></section>
